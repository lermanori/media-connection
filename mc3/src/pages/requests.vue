<template>
  <div>
        <app-request-field v-for="(prop,index) in Properties" :id="`${prop.id}`" :key="`${prop.id}`" :value="prop" @input="(arg)=>properties[index]=arg" >
        </app-request-field>
          ...
          <q-popup-proxy v-model="opened">
            <q-banner>
              ...
              <template v-slot:action>
                <q-input v-if="save==true" rounded outlined label="Enter Schema Name" v-model="schemaName" />
